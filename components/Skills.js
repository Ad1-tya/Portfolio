  ...
  const desList = skillsData.Designing.map(items => (
    <div className='skillscard relative hover:glisten '>
      <Image 
        ...
        layout='fill'
        alt='logos'
        className='greyeff max-w-[48px] max-h-[48px] m-auto inset-0'
      />
    </div> 
  ))
  ...
  const feList = skillsData.Frontend.map(items => (
    <div className='skillscard relative hover:glisten'>
      <Image 
        ...
        layout='fill'
        alt='logos'
        className='greyeff max-w-[48px] max-h-[40px] m-auto inset-0'
      />
    </div> 
  ))
    ...
    <div className='main flex flex-col'>
        ...
        <div className='flex gap-6 items-start content-center lg:gap-11 self-stretch flex-wrap flex-1'>
            ...
            <div className='flex flex-col items-start gap-5 flex-1'>
              <p className='text-neutral-500'>\\ REACT \\ CSS \\ HTML \\ NEXT \\ FRAMER MOTION \\</p>
              <div className='flex flex-wrap gap-4 lg:justify-between items-start content-start self-stretch sm:flex-nowrap'>{feList}</div>
